...
async function send(){
  ...
  pendingOrderNonce=(crypto.randomUUID?crypto.randomUUID():(Date.now()+"-"+Math.random().toString(36).slice(2)));
  setSendingState(true);

  const form=document.createElement("form");
  form.method="POST";
  form.action=EMAIL_WEB_APP_URL;
  form.target="orderSubmitFrame";
  form.style.display="none";

  const fields={
    order_text:orderText(),
    nonce:pendingOrderNonce,
  ...
  sendTimeout=setTimeout(()=>{
    if(!pendingOrderNonce)return;
    setSendingState(false);
    pendingOrderNonce="";
    toast("发送超时，请检查网络后再试一次");
  },15000);
}

window.addEventListener("message",(event)=>{
  const data=event.data||{};
  if(data.type!=="jimmy-kitchen-order-confirmed")return;
  if(!pendingOrderNonce || data.nonce!==pendingOrderNonce)return;
  ...
  if(data.ok){
    ...
    toast(data.message||"订单发送失败，请再试一次");
  }
});
